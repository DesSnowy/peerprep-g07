...
import { User, Mail, Award, Code, Save, Shield, Lock, Crown, Trash2, AlertTriangle, Loader2 } from "lucide-react";
import { useState, useEffect } from "react";
import { useAuth } from "@/app/context/AuthContext";
import { updateMyProfile } from "@/app/services/api";
...
export function UserProfileScreen() {
  ...
  // Keep local state in sync when user data loads/changes
  useEffect(() => {
  ...
  const handleSave = async () => {
    ...
    setSaveMessage(null);
    try {
      await updateMyProfile(username.trim());
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="pt-4 border-t-2 border-gray-200">
            <div className="mt-2 space-y-2">
              <Badge className="bg-blue-100 text-blue-800 border border-blue-300">
                {user?.access_role === "root-admin" ? "Root Admin" : user?.access_role === "admin" ? "Admin" : "Standard User"}
              </Badge>
        ...
        <div className="lg:col-span-2 border-4 border-gray-300 rounded-lg p-6 bg-white space-y-6">
          ...
          <div className="grid grid-cols-3 gap-4 pt-4 border-t-2 border-gray-200">
            ...
            <div className="text-center p-3 border-2 border-gray-300 rounded-lg">
              <div className="text-2xl font-bold text-gray-900">{user?.access_role ?? "-"}</div>
              <div className="text-xs text-gray-600">Role</div>
            </div>
            <div className="text-center p-3 border-2 border-gray-300 rounded-lg">
              <div className="text-2xl font-bold text-gray-900">{user?.created_at ? new Date(user.created_at).toLocaleDateString() : "-"}</div>
          ...
          <div className="flex justify-end gap-2 pt-4">
            <Button variant="outline" className="border-2 border-gray-300" onClick={() => { if (user) setUsername(user.username); setSaveMessage(null); }}>
              Cancel
            </Button>
            <Button className="bg-blue-600 hover:bg-blue-700 text-white disabled:opacity-50" onClick={handleSave} disabled={isSaving}>
